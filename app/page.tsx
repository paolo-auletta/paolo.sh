...
const inlineLinkClassName =
  "inline cursor-pointer font-medium whitespace-nowrap text-primary underline decoration-muted-foreground/25 decoration-1 underline-offset-3 transition-all hover:decoration-muted-foreground"
...
function LinkPreview({
  ...
  const trigger = internal ? (
    <Link href={href} className={inlineLinkClassName}>
      {children}
    </Link>
  ) : (
    <a href={href} {...externalLinkProps} className={inlineLinkClassName}>
      {children}
    </a>
  )

  // Artwork-backed previews stay plain links until their cover has loaded,
  // otherwise the card would fall back to a screenshot of an internal route.
  // Screenshots wait for the resolved theme so only one variant is requested.
  if ((internal && !src) || (!src && !isMounted)) {
    return trigger
  }
  ...
  return (
    <PreviewLinkCard
      ...
      width={width}
    >
      <PreviewLinkCardTrigger asChild>{trigger}</PreviewLinkCardTrigger>
      <PreviewLinkCardContent
        className="border-border/70 bg-background p-0"
        side="top"
        {...(internal ? {} : externalLinkProps)}
      >
        <PreviewLinkCardImage className="block h-auto" />
      </PreviewLinkCardContent>
    </PreviewLinkCard>
  )
}
...
export default function Page() {
  ...
  const [latestMoviePoster, setLatestMoviePoster] = useState<string>()
  const [latestMusicTitle, setLatestMusicTitle] = useState("Graduation")
  const [latestMusicArtwork, setLatestMusicArtwork] = useState<string>()
  ...
  useEffect(() => {
    ...
    fetch("/api/letterboxd/latest")
      ...
      .then((movie) => {
        ...
        if (movie.posterUrl) {
          setLatestMoviePoster(movie.posterUrl)
        }
      })
      .catch(() => {
    ...
    fetch("/api/music/latest")
      ...
      .then((music) => {
        ...
        if (music.artworkUrl) {
          setLatestMusicArtwork(music.artworkUrl)
        }
      })
      .catch(() => {
                ...
                >
                  ...
                  >
                    <Image
                      ...
                      height={106}
                      alt="Sapienza"
                      className="mr-1 mb-0.5 inline h-[1.05em] w-auto align-middle"
                    />
                    Sapienza
                  </a>{" "}
                  and managing{" "}
